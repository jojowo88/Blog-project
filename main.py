...
def admin_only(func):
    ...
    def wrapper_admin(*args, **kwargs):
        if current_user.is_authenticated and current_user.id == 1:
            # The below return function continues with the routing if it validates admin role
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            message = flash("Email already exists! Please log in instead.", category="error")
...
def show_post(post_id):
    ...
    all_comments = CommentPost.query.all()
    if form.validate_on_submit():
